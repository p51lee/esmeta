...
object RandomMutation extends Walker {
  ...
  def buildExpr(exprInfo: List[Either[(String, List[Boolean], Int), Ast]]): Ast = exprInfo match
    case Left((name, args, rhsIdx)) :: tail => Syntactic(name, args, rhsIdx, List(Some(buildExpr(tail))))
  ...
  val bindingId = buildExpr(List(
    Left("Identifier", ff, 0),
    Right(Lexical("IdentifierName \\ (ReservedWord)", "subway"))
  ))

  val initializer = buildExpr(List(
    Left("Initializer", tff, 0),
    Right(assignExpr)
  ))

  val declExpr = Syntactic("VariableDeclaration", tff, 0, List(Some(bindingId), Some(initializer)))

  val statement = buildExpr(List(
  ...
  override def walk(ast: Syntactic): Syntactic = {
    if (BaseUtils.randBool) {
      ...
      name match
        case "AssignmentExpression" => BaseUtils.choose(exprList).asInstanceOf[Syntactic]
        case "PrimaryExpression" => BaseUtils.choose(primExprList).asInstanceOf[Syntactic]
        case "Statement" => BaseUtils.choose(statementList).asInstanceOf[Syntactic]
        case "VariableDeclaration" => BaseUtils.choose(declarationList).asInstanceOf[Syntactic]
